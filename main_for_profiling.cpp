...
/*    while(model_flag[i] != 0 && model_flag[i] != 1){
    ...
    if(i == 1 && model_flag[i] == 0){ //get penalty params for elastic net
      while(l1 <= 0 && l2 <= 0 ){
        cout << "Please enter l1 penalty (eg .6)" << endl;
        cin >> l1;
        cout << "Please enter l2 penalty (eg .4)" << endl;
        cin >> l2;
        if(l1 <= 0 && l2 <= 0 ){
  ...
  //create gradient descent object
    ...
    if(model_flag[0] == 0 || model_flag[1] == 0 || model_flag[2] == 0){
      ...
      tol = 10e-3;
      /*while(tol <= 0){
        cout << endl << "What is the threshold for convergence of gradient descent? (eg 10e-4)" << endl;
        cin >> tol;
        if(tol <= 0){
      ...
      learnRate = 10e-3;
      /*while(learnRate <= 0){
        cout << endl << "What do you want the initial learning rate for gradient descent? (eg 10e-3)"  << endl;
        cin >> learnRate;
        if(learnRate <= 0){
